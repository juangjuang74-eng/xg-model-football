"""
train.py — xG Model dengan Farm-to-Feed Methodology

Pipeline:
  1. Load StatsBomb shots (dengan match_index kronologis)
  2. Feature engineering: geometris + player behaviour + interaction + temporal
  3. Gap-aware time-based CV (3 fold, gap 2 match)
  4. Train: LightGBM + CatBoost + Ensemble
  5. Evaluate: LogLoss, AUC, Brier Score per fold
  6. Train final model pada full dataset
  7. Visualisasi & simpan model
...
CV_N_SPLITS    = 3    # sama seperti Farm-to-Feed (3-fold)
CV_GAP         = 2    # 2 match gap (analog: 1-week gap)
CV_VAL_SIZE    = 6    # 6 match per val window
LGB_WEIGHT     = 0.5  # 50/50 ensemble (sama seperti Farm-to-Feed)
